...
begin
    ...
    set desconto_inss = 
		case
			when (renda <= 1412.00) then (renda*0.075)
...
begin
    ...
    set desconto_irpf = 
		 case
		when (renda <=2112) then (renda * 0.0)
...
delimiter ;
select *, calcula_irpf(salario) as IRPF, (salario-calcula_irpf(salario)) as liquido_renda from empregado;